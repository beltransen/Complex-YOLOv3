...
class YOLOLayer(nn.Module):
    ...
    def forward(self, x, targets=None, img_dim=None):
        ...
        prediction = (
            x.view(num_samples, self.num_anchors, self.num_classes + 7 + 2, grid_size, grid_size) # +2 TODO
            .permute(0, 1, 3, 4, 2)
            .contiguous()
        )
        ...
        pred_cls = torch.sigmoid(prediction[..., 7:7+self.num_classes])  # Cls pred.
        ...
        pred_boxes = FloatTensor(prediction[..., :6+2].shape)
        ...
        output = torch.cat(
            (
                ...
                pred_boxes[..., 6:].view(num_samples, -1, 2)
            ),
            -1,
        )
        ...
        if targets is None:
            ...
            iou_scores, class_mask, obj_mask, noobj_mask, tx, ty, tw, th, tim, tre, tcls, tconf, tz, theight = build_targets(
            ...
            loss_height = self.mse_loss(height[obj_mask], theight[obj_mask])
            ...
            total_loss = loss_x + loss_y + loss_w + loss_h + loss_eular + loss_conf + loss_cls + loss_z + loss_height
            ...
            self.metrics = {
                ...
                "height": to_cpu(loss_height).item(),
